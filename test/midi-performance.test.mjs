...
test('Standard MIDI tempo, running status, notes, and sustain become timed controls', () => {
 const bytes = midiFile([0x00, 0xff, 0x51, 0x03, 0x07, 0xa1, 0x20, 0x00, 0x90, 60, 100, 0x00, 64, 80, 0x81, 0x70, 0xb0, 64, 127, 0x81, 0x70, 0x80, 60, 32, 0x00, 64, 0, 0x83, 0x60, 0xb0, 64, 0, 0x00, 0xff, 0x2f, 0]); const parsed = parseStandardMidi(bytes); assert.equal(parsed.noteCount, 2); assert.equal(parsed.trackCount, 1); assert.deepEqual(parsed.controls.map(({ type, seconds }) => [type, seconds]), [['noteOn', 0], ['noteOn', 0], ['sustain', .25], ['noteOff', .5], ['noteOff', .5], ['sustain', 1]]); assert.equal(parsed.controls[0].midi, 60); assert.equal(parsed.controls[1].midi, 64);
});

test('parsed MIDI renders through the persistent realtime voice engine', () => {
 const bytes = midiFile([0x00, 0x90, 69, 100, 0x83, 0x60, 0x80, 69, 0, 0x00, 0xff, 0x2f, 0]); const parsed = parseStandardMidi(bytes), rendered = renderMidiPerformance(parsed, { tailSeconds: .2, polyphony: 4 }); assert.equal(rendered.sampleRate, 44_100); assert.ok(rendered.mono.some((sample) => sample !== 0)); assert.ok(rendered.mono.every(Number.isFinite)); assert.equal(rendered.clippedFrames, 0); assert.equal(rendered.maximumVoices, 1);
});
