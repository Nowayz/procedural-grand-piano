...
import { applyConvolverReverb, applyStereoLoudnessCeiling } from './convolution-reverb.mjs';
...
if (!source) throw new Error('usage: node tools/render-midi-realtime.mjs <score.mid> [output.wav] [impulse.wav]');
...
const impulsePath = process.argv[4] ? path.resolve(process.argv[4]) : fileURLToPath(DEFAULT_REVERB_IR_URL);
...
const impulse = await readWav(impulsePath, { preserveChannels: true });
console.log(`applying ${path.basename(impulsePath)} (${Math.round(DEFAULT_REVERB_WET * 100)}% wet)`);
...
const loudness = applyStereoLoudnessCeiling(left, right, { sampleRate: rendered.sampleRate });
await mkdir(path.dirname(destination), { recursive: true });
await writeStereoPcm16Wav(destination, left, right, rendered.sampleRate);
console.log(`wrote ${path.relative(root, destination)} (${(rendered.mono.length / rendered.sampleRate).toFixed(2)} s, ${synthesisSeconds.toFixed(2)} s synthesis, ${(rendered.mono.length / rendered.sampleRate / synthesisSeconds).toFixed(1)}x realtime, ${rendered.maximumVoices} peak voices, ${rendered.clippedFrames} clipped source frames, reverb gain ${reverb.normalizationScale.toFixed(4)}, maximum loudness reduction ${loudness.maximumReductionDb.toFixed(1)} dB)`);
