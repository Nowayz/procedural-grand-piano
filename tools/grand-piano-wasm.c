...
typedef struct {
 ...
 uint32_t mode_cutoff[MAX_MODES], note_id, age, release_at, damper_at, release_end_at, noise_state, damper_noise_state, body_noise_state, quiet_samples, inaudible_samples;
 uint64_t serial;
 ...
 double sample_rate, frequency, strike_velocity, hammer_speed, release_speed, midi, reg, velocity_gain, hammer_lowpass_step, hammer_lowpass, mechanical_lowpass, damper_lowpass, mechanical_lowpass_step, damper_lowpass_step, damper_noise_gain, body_damper_free_pole, body_damper_regulated_pole, diffuse_body_gain, output_envelope, output_peak, release_envelope_pole, inaudible_energy, inaudible_bridge_energy, felt_presence_radiation, felt_air_radiation, thump_frequency, body_rise_pole, body_tail_pole, plate_rise_pole, plate_tail_pole, felt_rise_pole, felt_tail_pole, presence_rise_pole, presence_tail_pole, early_rise_pole, early_tail_pole, body_grain_tail_pole, thump_tail_pole, thump_rise_pole, damper_rise_pole, damper_tail_pole, diffuse_plate_register, upper_bridge_plate, diffuse_low_body_scale, string_mix, previous_input, dc_blocker, bridge_output, previous_sympathetic_force, key_position, stereo_lowpass, stereo_lowpass_step, stereo_position, radiation_lobe, stereo_side, body_rise_value, body_tail_value, plate_rise_value, plate_tail_value, felt_rise_value, felt_tail_value, presence_rise_value, presence_tail_value, early_rise_value, early_tail_value, body_grain_tail_value, thump_tail_value, thump_rise_value, damper_rise_value, damper_tail_value, dc_pole;
} Voice;
...
static Voice *claim_voice(void) { Voice *candidate = 0; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (!voices[index].active) return voices + index; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (voices[index].released && quieter_voice(voices + index, candidate)) candidate = voices + index; if (candidate) return candidate; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (!voices[index].key_down && quieter_voice(voices + index, candidate)) candidate = voices + index; if (candidate) return candidate; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (quieter_voice(voices + index, candidate)) candidate = voices + index; return candidate; }
...
static void render_segment(uint32_t start, uint32_t end) { for (uint32_t frame = start; frame < end; frame += 1) { double previous_bus = isfinite(sympathetic_bridge_bus) ? sympathetic_bridge_bus : 0, next_bus = 0, coupling_scale = active_voices > 32 ? 32.0 / active_voices : 1; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) { Voice *target = voices + index; if (!target->active) continue; double other_bridge_force = .002 * coupling_scale * (previous_bus - target->bridge_output), sample = step_voice(target, other_bridge_force); if (!isfinite(sample) || !isfinite(target->bridge_output) || fabs(target->bridge_output) > 16) { memset(target, 0, sizeof(Voice)); active_voices -= 1; continue; } realtime_mix[frame] += sample; realtime_side[frame] += target->stereo_side; next_bus += target->bridge_output; if (target->quiet_samples >= (uint32_t)round(RELEASE_QUIET_SECONDS * target->sample_rate) || target->inaudible_samples >= (uint32_t)round(INAUDIBLE_SECONDS * target->sample_rate)) { target->active = 0; active_voices -= 1; } } sympathetic_bridge_bus = isfinite(next_bus) ? next_bus : 0; } }
