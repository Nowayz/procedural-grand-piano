...
typedef struct {
 double state[STATE_COUNT], modes[MODE_ROWS][MAX_MODES], impact_state[17][IMPACT_COUNT], duplex[2][5];
 double phase_drive[3][MAX_MODES];
 uint8_t mode_delays[2][MAX_MODES], active, released, key_down, deferred_release, key_motion_controlled;
 uint32_t mode_cutoff[MAX_MODES], note_id, age, release_at, damper_at, release_end_at, noise_state, damper_noise_state, body_noise_state, quiet_samples;
 uint64_t serial;
 int mode_count, active_mode_count, impact_body_count, impact_attack_count, hammer_samples, release_samples, damper_stage_samples, damper_settle_samples, damper_noise_samples, start_fade_samples, hammer_noise_samples, strike_delay_samples;
 double sample_rate, frequency, strike_velocity, release_speed, midi, reg, velocity_gain, hammer_lowpass_step, hammer_lowpass, mechanical_lowpass, damper_lowpass, mechanical_lowpass_step, damper_lowpass_step, damper_noise_gain, body_damper_free_pole, body_damper_regulated_pole, diffuse_body_gain, output_envelope, output_peak, release_envelope_pole, felt_presence_radiation, felt_air_radiation, thump_frequency, body_rise_pole, body_tail_pole, plate_rise_pole, plate_tail_pole, felt_rise_pole, felt_tail_pole, presence_rise_pole, presence_tail_pole, early_rise_pole, early_tail_pole, body_grain_tail_pole, thump_tail_pole, thump_rise_pole, damper_rise_pole, damper_tail_pole, diffuse_plate_register, upper_bridge_plate, diffuse_low_body_scale, string_mix, previous_input, dc_blocker, bridge_output, previous_sympathetic_force, key_position, stereo_lowpass, stereo_lowpass_step, stereo_position, radiation_lobe, stereo_side, body_rise_value, body_tail_value, plate_rise_value, plate_tail_value, felt_rise_value, felt_tail_value, presence_rise_value, presence_tail_value, early_rise_value, early_tail_value, body_grain_tail_value, thump_tail_value, thump_rise_value, damper_rise_value, damper_tail_value, dc_pole;
} Voice;
...
static Voice offline_voice, voices[MAX_VOICES], *voice = &offline_voice;
...
static uint32_t queued_events, active_voices, realtime_voice_limit = MAX_VOICES;
static uint64_t next_serial;
static double sustain_lift, una_corda_position, sympathetic_bridge_bus, left_microphone_state, right_microphone_state, left_microphone_step, right_microphone_step;
...
#define impact_body_count (voice->impact_body_count)
#define impact_attack_count (voice->impact_attack_count)
 ...
 *decay_seconds = .035 * pow(*frequency / 58, -.31);
 double x = log(*frequency / 2350), width = x < 0 ? .13 : .17, normalized_x = x / width, bridge_presence = 1 + 31 * exp(-.5 * normalized_x * normalized_x);
 double participation = (mode - 2 * floor(mode * .5)) < 1 ? 1 : -1;
 *gain = participation * .065 * pow(*frequency / 58, -.42) * bridge_presence;
}
...
static double string_inharmonicity(double midi) {
 return exp(-.07643470205 * midi - 6.682289773) + exp(.07965147182 * midi - 12.91563584);
}
...
static void initialize_filters(double sample_rate) {
 ...
 for (int index = 0; index < SOUND_FILTER_COUNT; index += 1) { double frequency, q, gain; soundboard_profile(index, &frequency, &q, &gain); double omega = TWO_PI * frequency / sample_rate, alpha = sin(omega) / (2 * q), inverse_a0 = 1 / (1 + alpha); soundboard_filters[index][0] = alpha * inverse_a0; soundboard_filters[index][1] = 0; soundboard_filters[index][2] = -alpha * inverse_a0; soundboard_filters[index][3] = -2 * cos(omega) * inverse_a0; soundboard_filters[index][4] = (1 - alpha) * inverse_a0; soundboard_filters[index][9] = gain; }
...
static double unison_phase(uint64_t serial, int midi_key, int string_index, int string_count) {
 ...
 return .12 * (values[string_index] - mean / string_count);
}
...
static int create_string_modes(double frequency, double velocity, double midi, double reg, uint64_t serial) {
 ...
 for (int partial = 1; partial <= 192; partial += 1) {
  ...
  for (int string_index = 0; string_index < string_count; string_index += 1) {
   ...
   double string_weight = lerp(equal_weight, string_count == 1 ? 1 : string_count == 2 ? (string_index == 0 ? .47 : .53) : (string_index == 0 ? .09 : string_index == 1 ? .46 : .45), unison_inequality), shifted_contact = string_count == 1 ? .82 : string_count == 2 ? (string_index ? .32 : 1) : (string_index == 2 ? .08 : 1); string_weight *= lerp(1, shifted_contact, una_corda_position);
   ...
   mode_cutoff[mode_count] = decay_samples(fabs(weighted_amplitude) * fmax(fast_fraction * vertical_second_partial_boost, fmax(1 - fast_fraction, polarization_strength)), fmax(fast_pole, fmax(slow_pole, polarization_pole))); mode_count += 1;
  }
 }
 return mode_count;
}
...
static inline double filter_free_modes(int count, double damper_strength, double free_return_blend, double sympathetic_force) {
 v128_t sum = wasm_f64x2_splat(0), one = wasm_f64x2_splat(1), two = wasm_f64x2_splat(2), three = wasm_f64x2_splat(3), strength = wasm_f64x2_splat(damper_strength), blend = wasm_f64x2_splat(free_return_blend), horizontal_fraction = wasm_f64x2_splat(.45), force = wasm_f64x2_splat(sympathetic_force), previous_force = wasm_f64x2_splat(voice->previous_sympathetic_force); int index = 0;
 for (; index + 1 < count; index += 2) { v128_t fast = forced_string_step(0, index, force, previous_force), slow = forced_string_step(5, index, force, previous_force), termination = wasm_v128_load(modes[21] + index); termination = wasm_f64x2_mul(termination, wasm_v128_load(modes[22] + index)); wasm_v128_store(modes[21] + index, termination); v128_t vertical = wasm_f64x2_add(fast, wasm_f64x2_mul(slow, wasm_f64x2_sub(one, termination))), horizontal = forced_string_step(10, index, force, previous_force), bridge = wasm_v128_load(modes[15] + index), bridge_step = wasm_v128_load(modes[16] + index); bridge = wasm_f64x2_add(bridge, wasm_f64x2_mul(wasm_f64x2_sub(one, bridge), bridge_step)); wasm_v128_store(modes[15] + index, bridge); v128_t transmission = wasm_f64x2_mul(wasm_f64x2_mul(bridge, bridge), wasm_f64x2_sub(three, wasm_f64x2_mul(two, bridge))), gain = wasm_v128_load(modes[17] + index), horizontal_gain = wasm_v128_load(modes[20] + index); if (damper_strength > 0) { v128_t regulated_pole = wasm_v128_load(modes[19] + index), free_pole = wasm_v128_load(modes[18] + index), target_pole = wasm_f64x2_add(regulated_pole, wasm_f64x2_mul(wasm_f64x2_sub(free_pole, regulated_pole), blend)), effective_pole = wasm_f64x2_add(one, wasm_f64x2_mul(wasm_f64x2_sub(target_pole, one), strength)), horizontal_pole = wasm_f64x2_add(one, wasm_f64x2_mul(wasm_f64x2_sub(effective_pole, one), horizontal_fraction)); gain = wasm_f64x2_mul(gain, effective_pole); horizontal_gain = wasm_f64x2_mul(horizontal_gain, horizontal_pole); wasm_v128_store(modes[17] + index, gain); wasm_v128_store(modes[20] + index, horizontal_gain); } v128_t value = wasm_f64x2_add(wasm_f64x2_mul(vertical, gain), wasm_f64x2_mul(horizontal, horizontal_gain)); sum = wasm_f64x2_add(sum, wasm_f64x2_mul(value, transmission)); }
 double lanes[2]; wasm_v128_store(lanes, sum); double result = lanes[0] + lanes[1]; if (index < count) { double previous_force_scalar = voice->previous_sympathetic_force, fast = modes[2][index] * modes[0][index] + modes[3][index] * modes[1][index] + modes[4][index] * sympathetic_force + phase_drive[0][index] * previous_force_scalar; modes[1][index] = modes[0][index]; modes[0][index] = fast; double slow = modes[7][index] * modes[5][index] + modes[8][index] * modes[6][index] + modes[9][index] * sympathetic_force + phase_drive[1][index] * previous_force_scalar; modes[6][index] = modes[5][index]; modes[5][index] = slow; modes[21][index] *= modes[22][index]; double polarization = modes[12][index] * modes[10][index] + modes[13][index] * modes[11][index] + modes[14][index] * sympathetic_force + phase_drive[2][index] * previous_force_scalar; modes[11][index] = modes[10][index]; modes[10][index] = polarization; modes[15][index] += (1 - modes[15][index]) * modes[16][index]; double bridge = modes[15][index], gain = modes[17][index], horizontal_gain = modes[20][index]; if (damper_strength > 0) { double target_pole = lerp(modes[19][index], modes[18][index], free_return_blend), effective_pole = lerp(1, target_pole, damper_strength); gain *= effective_pole; horizontal_gain *= lerp(1, effective_pole, .45); modes[17][index] = gain; modes[20][index] = horizontal_gain; } result += ((fast + slow * (1 - modes[21][index])) * gain + polarization * horizontal_gain) * bridge * bridge * (3 - 2 * bridge); } voice->previous_sympathetic_force = sympathetic_force; return result;
}
...
static double filter_driven_strings(int mode_count, int strike_index, int hammer_samples, double damper_strength, double free_return_blend) {
 double strings = 0; for (int index = 0; index < mode_count; index += 1) { int force_index = strike_index - mode_delays[0][index], polarization_force_index = strike_index - mode_delays[1][index]; double string_force = force_index >= 0 && force_index < hammer_samples ? state[FORCE_O + force_index] : 0, previous_string_force = force_index > 0 && force_index <= hammer_samples ? state[FORCE_O + force_index - 1] : 0, polarization_force = polarization_force_index >= 0 && polarization_force_index < hammer_samples ? state[FORCE_O + polarization_force_index] : 0, previous_polarization_force = polarization_force_index > 0 && polarization_force_index <= hammer_samples ? state[FORCE_O + polarization_force_index - 1] : 0, fast = resonate_string(index, 0, string_force, previous_string_force), slow = resonate_string(index, 5, string_force, previous_string_force), polarization = resonate_string(index, 10, polarization_force, previous_polarization_force); modes[21][index] *= modes[22][index]; modes[15][index] += (1 - modes[15][index]) * modes[16][index]; if (damper_strength > 0) { double target_pole = lerp(modes[19][index], modes[18][index], free_return_blend), effective_pole = lerp(1, target_pole, damper_strength); modes[17][index] *= effective_pole; modes[20][index] *= lerp(1, effective_pole, .45); } strings += ((fast + slow * (1 - modes[21][index])) * modes[17][index] + polarization * modes[20][index]) * smoothstep(modes[15][index]); } return strings;
}
...
static void start_voice(Voice *target, uint32_t note_id, double note_hz, double velocity, double sample_rate, uint64_t serial) {
 memset(target, 0, sizeof(*target)); voice = target; target->sample_rate = sample_rate; initialize_filters(sample_rate); target->frequency = clamp(note_hz, 27.5, 4186.009044809578); target->strike_velocity = clamp(velocity, 0, 1); target->midi = 69 + 12 * log2(target->frequency / 440); target->reg = clamp((target->midi - 21) / 87, 0, 1); target->note_id = note_id; target->serial = serial; target->active = target->strike_velocity > 0; target->key_down = target->active; target->key_position = target->active; target->release_at = UINT32_MAX; target->damper_at = UINT32_MAX; target->release_end_at = UINT32_MAX;
 target->hammer_samples = hammer_force_length(target->strike_velocity, target->reg, target->frequency); create_hammer_force(target->hammer_samples, target->strike_velocity, target->reg); target->mode_count = create_string_modes(target->frequency, target->strike_velocity, target->midi, target->reg, serial); target->active_mode_count = target->mode_count; create_duplex_modes(target); create_impact_soundboard(target->strike_velocity, target->reg, target->midi); uint32_t noise_seed = seed_from_arguments(target->frequency, target->strike_velocity); target->noise_state = noise_seed ? noise_seed : 0x6d2b79f5; target->damper_noise_state = (noise_seed ^ 0x85ebca6b) ? (noise_seed ^ 0x85ebca6b) : 0x6d2b79f5; target->body_noise_state = (noise_seed ^ 0x9e3779b9) ? (noise_seed ^ 0x9e3779b9) : 0x6d2b79f5;
 double top_velocity_transition = transition((target->midi - 99) / 9), upper_action_leverage = bell(target->midi, 105, 2.2, 2), velocity_exponent = .28 + target->reg + .64 * top_velocity_transition + .45 * upper_action_leverage + .82 * transition((36 - target->midi) / 15), bass_velocity_voicing = transition((48 - target->midi) / 27), bass_velocity_bump_db = 7 * bass_velocity_voicing * bell(target->strike_velocity, .38, .25, 2), bass_compensation = 1 + .48 * clamp((45 - target->midi) / 24, 0, 1), bass_trim = lerp(.25, 1, transition((target->midi - 21) / 27)); target->damper_stage_samples = fmax(1, round(REGULATED_DAMPER_STAGE_SECONDS * sample_rate)); target->damper_noise_samples = fmax(1, round(.12 * sample_rate)); target->start_fade_samples = round(lerp(160, 32, transition((target->midi - 21) / 9)) * sample_rate / SAMPLE_RATE); target->velocity_gain = .3 * bass_compensation * bass_trim * pow(10, register_radiation_db(target->midi) / 20) * pow(10, bass_velocity_bump_db / 20) * pow(target->strike_velocity, velocity_exponent) * (.84 + .16 * target->strike_velocity);
...
static void release_voice(Voice *target, double velocity) {
 if (!target->active || target->released) return; target->released = 1; target->deferred_release = 0; target->release_at = target->age; target->release_speed = normalize_release_speed(velocity); if (!target->key_motion_controlled) target->key_position = 0; target->quiet_samples = 0;
...
static inline void catch_damper(Voice *target) { if (!target->active || target->key_down || !target->released || target->midi >= FIRST_UNDAMPED_MIDI - .5) return; target->released = 0; target->deferred_release = 1; target->release_at = UINT32_MAX; target->damper_at = UINT32_MAX; target->release_end_at = UINT32_MAX; target->quiet_samples = 0; }

static double step_voice(Voice *target, double sympathetic_force) {
 voice = target; int sample_index = target->age, strike_index = sample_index - target->strike_delay_samples, released = target->released; uint32_t damper_index = 0; double damper_strength = 0, free_return_blend = 1, body_damper_pole = 1; if (released && target->damper_at != UINT32_MAX && (uint32_t)sample_index >= target->damper_at) { damper_index = sample_index - target->damper_at; damper_strength = transition((double)(damper_index + 1) / target->damper_settle_samples) * damper_interaction_strength(target); free_return_blend = damper_index < (uint32_t)target->damper_stage_samples ? target->release_speed : 1; double key_damper_lift = transition((target->key_position - .18) / .5); damper_strength *= 1 - key_damper_lift; body_damper_pole = lerp(1, lerp(target->body_damper_regulated_pole, target->body_damper_free_pole, free_return_blend), damper_strength); } double force = strike_index >= 0 && strike_index < target->hammer_samples ? state[FORCE_O + strike_index] : 0, diffuse_body = 0;
 if (body_damper_pole < 1) target->diffuse_body_gain *= body_damper_pole; if (strike_index >= 0) { target->body_noise_state ^= target->body_noise_state << 13; target->body_noise_state ^= target->body_noise_state >> 17; target->body_noise_state ^= target->body_noise_state << 5; double body_white = (double)target->body_noise_state / 4294967296.0 * 2 - 1, body_grain = filter_chain(body_white, noise_filters, NOISE_O, 9, 12), plate_grain = filter_chain(body_white, noise_filters, NOISE_O, 12, 14), body_rise = 1 - target->body_rise_value, body_tail = target->body_tail_value, plate_rise = 1 - target->plate_rise_value, plate_tail = target->plate_tail_value; diffuse_body = target->diffuse_body_gain * (.015 * (1.7 - 1.4 * target->strike_velocity) * (.55 + .45 * target->reg) * target->diffuse_low_body_scale * body_grain * body_rise * body_tail + .002 * target->diffuse_plate_register * target->upper_bridge_plate * plate_grain * plate_rise * plate_tail); }
 while (target->active_mode_count > 0 && strike_index >= 0 && (uint32_t)strike_index >= mode_cutoff[target->active_mode_count - 1]) target->active_mode_count -= 1; double open_string_force = target->key_down || damper_contact(sustain_lift) == 0 ? sympathetic_force : 0, strings = strike_index < target->hammer_samples + 3 ? filter_driven_strings(target->active_mode_count, strike_index, target->hammer_samples, damper_strength, free_return_blend) : filter_free_modes(target->active_mode_count, damper_strength, free_return_blend, open_string_force), hammer = 0;
 if (strike_index >= 0 && strike_index < target->hammer_noise_samples) { target->noise_state ^= target->noise_state << 13; target->noise_state ^= target->noise_state >> 17; target->noise_state ^= target->noise_state << 5; double white = (double)target->noise_state / 4294967296.0 * 2 - 1; target->hammer_lowpass += target->hammer_lowpass_step * (white - target->hammer_lowpass); double felt_lowpassed = filter_chain(white, noise_filters, NOISE_O, 0, 3), felt_presence = filter_biquad(felt_lowpassed, noise_filters[3], NOISE_O + 12), felt_air = filter_chain(white, noise_filters, NOISE_O, 4, 6), body_grain = filter_chain(white, noise_filters, NOISE_O, 6, 9); target->mechanical_lowpass += target->mechanical_lowpass_step * (white - target->mechanical_lowpass); double lingering_felt = felt_lowpassed * (1 - target->felt_rise_value) * target->felt_tail_value, lingering_presence = felt_presence * (1 - target->presence_rise_value) * target->presence_tail_value, early_presence = felt_presence * (1 - target->early_rise_value) * target->early_tail_value, lingering_air = felt_air * (1 - target->presence_rise_value) * target->presence_tail_value, body_grain_envelope = (1 - target->body_rise_value) * target->body_grain_tail_value; hammer = .024 * pow(target->strike_velocity, 1.55) * lingering_felt + .05 * pow(target->strike_velocity, 1.75) * target->felt_presence_radiation * lingering_presence + .04 * pow(target->strike_velocity, 1.62) * target->felt_presence_radiation * early_presence + .0022 * pow(target->strike_velocity, 1.9) * target->felt_air_radiation * lingering_air + .2 * pow(target->strike_velocity, .45) * target->reg * target->reg * body_grain * body_grain_envelope; if (strike_index < target->hammer_samples) { double collision_shape = state[FORCE_O + strike_index] * target->hammer_samples, collision = sqrt(collision_shape), felt = target->hammer_lowpass * collision, felt_contact = felt_presence * collision, air_contact = felt_air * collision, mechanical_impact = .03 * pow(target->strike_velocity, 1.2) * target->mechanical_lowpass * collision, thump = sin(TWO_PI * target->thump_frequency * strike_index / target->sample_rate) * target->thump_tail_value * (1 - target->thump_rise_value); hammer += (.005 * pow(target->strike_velocity, 1.35) * felt + .012 * pow(target->strike_velocity, 1.55) * target->felt_presence_radiation * felt_contact + .006 * pow(target->strike_velocity, 1.9) * target->felt_air_radiation * air_contact + mechanical_impact + .006 * thump) * (.72 + .28 * target->reg); } }
 ...
 target->stereo_lowpass += target->stereo_lowpass_step * (sample - target->stereo_lowpass); double high_radiation = sample - target->stereo_lowpass; target->stereo_side = (.14 * target->stereo_position + .075 * target->radiation_lobe) * high_radiation + .018 * target->stereo_position * target->stereo_lowpass; double absolute_sample = fabs(sample); target->output_peak = fmax(target->output_peak, absolute_sample); target->output_envelope = fmax(absolute_sample, target->output_envelope * target->release_envelope_pole); int release_spent = released && target->release_end_at != UINT32_MAX && (uint32_t)sample_index >= target->release_end_at, natural_modes_spent = strike_index >= 0 && target->active_mode_count == 0; if ((release_spent || natural_modes_spent) && target->output_envelope <= fmax(1e-9, target->output_peak * 1e-4)) target->quiet_samples += 1; else target->quiet_samples = 0; target->age += 1; return sample;
}
...
static Voice *find_voice(uint32_t note_id) { Voice *newest = 0; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (voices[index].active && voices[index].note_id == note_id && (!newest || voices[index].serial > newest->serial)) newest = voices + index; return newest; }
static Voice *claim_voice(void) { Voice *candidate = 0; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (!voices[index].active) return voices + index; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (voices[index].released && (!candidate || voices[index].serial < candidate->serial)) candidate = voices + index; if (candidate) return candidate; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (!voices[index].key_down && (!candidate || voices[index].serial < candidate->serial)) candidate = voices + index; if (candidate) return candidate; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (!candidate || voices[index].serial < candidate->serial) candidate = voices + index; return candidate; }
static void apply_event(const Event *event) {
 if (event->type == 1) { Voice *target = find_voice(event->note_id); if (event->velocity <= 0) { if (target) { target->key_down = 0; target->key_position = 0; if (damper_contact(sustain_lift) == 0 && target->midi < FIRST_UNDAMPED_MIDI - .5) { target->release_speed = DEFAULT_RELEASE_SPEED; target->deferred_release = 1; } else release_voice(target, DEFAULT_RELEASE_SPEED); } return; } double strike_velocity = event->velocity; if (target && target->key_motion_controlled) { double action_return = transition((.72 - target->key_position) / .52); strike_velocity *= lerp(.72, 1, action_return); } if (target) { target->key_down = 0; if (damper_contact(sustain_lift) == 0 && target->midi < FIRST_UNDAMPED_MIDI - .5) { target->release_speed = DEFAULT_RELEASE_SPEED; target->deferred_release = 1; } else release_voice(target, DEFAULT_RELEASE_SPEED); } target = claim_voice(); if (!target->active) active_voices += 1; start_voice(target, event->note_id, event->value, strike_velocity, realtime_rate, ++next_serial); return; }
 if (event->type == 2) { Voice *target = find_voice(event->note_id); if (target) { target->key_down = 0; if (damper_contact(sustain_lift) == 0 && target->midi < FIRST_UNDAMPED_MIDI - .5) { target->release_speed = normalize_release_speed(event->value); target->deferred_release = 1; } else release_voice(target, event->value); } return; }
 if (event->type == 3) { double old_contact = damper_contact(sustain_lift), new_lift = clamp(event->value, 0, 1), new_contact = damper_contact(new_lift); if (old_contact > 0 && new_contact == 0) for (uint32_t index = 0; index < realtime_voice_limit; index += 1) catch_damper(voices + index); if (old_contact == 0 && new_contact > 0) for (uint32_t index = 0; index < realtime_voice_limit; index += 1) if (voices[index].active && voices[index].deferred_release && !voices[index].key_down) release_voice(voices + index, DEFAULT_RELEASE_SPEED); sustain_lift = new_lift; }
 if (event->type == 4) una_corda_position = clamp(event->value, 0, 1);
 if (event->type == 5) { Voice *target = find_voice(event->note_id); if (!target) return; double position = clamp(event->value, 0, 1); target->key_motion_controlled = 1; target->key_position = position; if (position < .08 && target->key_down) { target->key_down = 0; if (damper_contact(sustain_lift) == 0 && target->midi < FIRST_UNDAMPED_MIDI - .5) { target->release_speed = normalize_release_speed(event->velocity); target->deferred_release = 1; } else release_voice(target, event->velocity); } else if (position > .68 && target->released) { catch_damper(target); target->key_down = 1; } }
}
static void render_segment(uint32_t start, uint32_t end) { for (uint32_t frame = start; frame < end; frame += 1) { double previous_bus = sympathetic_bridge_bus, next_bus = 0; for (uint32_t index = 0; index < realtime_voice_limit; index += 1) { Voice *target = voices + index; if (!target->active) continue; double other_bridge_force = .002 * (previous_bus - target->bridge_output), sample = step_voice(target, other_bridge_force); realtime_mix[frame] += sample; realtime_side[frame] += target->stereo_side; next_bus += target->bridge_output; if (target->quiet_samples >= (uint32_t)round(RELEASE_QUIET_SECONDS * target->sample_rate)) { target->active = 0; active_voices -= 1; } } sympathetic_bridge_bus = next_bus; } }
...
__attribute__((export_name("rt_reset"))) int reset(double sample_rate, uint32_t voice_limit) { if (!isfinite(sample_rate) || sample_rate < 32000 || sample_rate > 96000 || voice_limit < 1 || voice_limit > MAX_VOICES) return -1; memset(voices, 0, sizeof(voices)); memset(realtime_output, 0, sizeof(realtime_output)); memset(realtime_output_left, 0, sizeof(realtime_output_left)); memset(realtime_output_right, 0, sizeof(realtime_output_right)); memset(realtime_mix, 0, sizeof(realtime_mix)); memset(realtime_side, 0, sizeof(realtime_side)); realtime_rate = sample_rate; realtime_voice_limit = voice_limit; queued_events = 0; active_voices = 0; next_serial = 0; sustain_lift = 0; una_corda_position = 0; sympathetic_bridge_bus = 0; left_microphone_state = 0; right_microphone_state = 0; left_microphone_step = 1 - exp(-TWO_PI * 12000 / sample_rate); right_microphone_step = 1 - exp(-TWO_PI * 10500 / sample_rate); voice = &offline_voice; initialize_filters(sample_rate); return 0; }
...
__attribute__((export_name("rt_process"))) int process(uint32_t frame_count) { if (frame_count > BLOCK_SIZE) return -1; memset(realtime_mix, 0, frame_count * sizeof(double)); memset(realtime_side, 0, frame_count * sizeof(double)); initialize_filters(realtime_rate); uint32_t cursor = 0, event_index = 0; while (event_index < queued_events && events[event_index].offset < frame_count) { uint32_t offset = events[event_index].offset; render_segment(cursor, offset); do { apply_event(events + event_index); event_index += 1; } while (event_index < queued_events && events[event_index].offset == offset); cursor = offset; } render_segment(cursor, frame_count); for (uint32_t frame = 0; frame < frame_count; frame += 1) { double mid = realtime_mix[frame], side = realtime_side[frame], left = mid + side, right = mid - side; left_microphone_state += left_microphone_step * (left - left_microphone_state); right_microphone_state += right_microphone_step * (right - right_microphone_state); realtime_output[frame] = fmin(.94, fmax(-.94, mid)); realtime_output_left[frame] = fmin(.94, fmax(-.94, .88 * left + .12 * left_microphone_state)); realtime_output_right[frame] = fmin(.94, fmax(-.94, .86 * right + .14 * right_microphone_state)); } uint32_t future_count = queued_events - event_index; if (future_count) memmove(events, events + event_index, future_count * sizeof(Event)); queued_events = future_count; for (uint32_t index = 0; index < queued_events; index += 1) events[index].offset -= frame_count; return frame_count; }
