...
#ifndef PIANO_DENSE_MODE_CUTOFF_SCALE
/* In a dense mix the conservative -120 dB per-component lifetime can end a
 * little early while remaining below both the absolute and masking floors. */
#define PIANO_DENSE_MODE_CUTOFF_SCALE .8
#endif
#ifndef PIANO_VERY_DENSE_MODE_CUTOFF_SCALE
#define PIANO_VERY_DENSE_MODE_CUTOFF_SCALE .7
#endif
...
static inline uint32_t voice_mode_cutoff_at(const Voice *target, int index) { return target->mode_cutoff[index]; }
...
static void apply_event(const Event *event) {
 if (event->type == 1) { Voice *identity = find_voice(event->note_id); if (event->velocity <= 0) { if (identity) { identity->key_down = 0; identity->key_position = 0; if (damper_contact(sustain_lift) == 0 && identity->midi < FIRST_UNDAMPED_MIDI - .5) { identity->release_speed = DEFAULT_RELEASE_SPEED; identity->deferred_release = 1; } else release_voice(identity, DEFAULT_RELEASE_SPEED); } return; } Voice *target = find_key_voice(event->value); double strike_velocity = event->velocity; if (identity && identity->key_motion_controlled) { double action_return = transition((.72 - identity->key_position) / .52); strike_velocity *= lerp(.72, 1, action_return); } if (identity && identity != target) { identity->key_down = 0; if (damper_contact(sustain_lift) == 0 && identity->midi < FIRST_UNDAMPED_MIDI - .5) { identity->release_speed = DEFAULT_RELEASE_SPEED; identity->deferred_release = 1; } else release_voice(identity, DEFAULT_RELEASE_SPEED); } if (target) { retrigger_voice(target, event->note_id, strike_velocity, ++next_serial); return; } target = claim_voice(); if (!target->active) active_voices += 1; start_voice(target, event->note_id, event->value, strike_velocity, realtime_rate, ++next_serial); return; }
 ...
 if (event->type == 3) { double old_contact = damper_contact(sustain_lift), new_lift = clamp(event->value, 0, 1), new_contact = damper_contact(new_lift); if (old_contact > 0 && new_contact == 0) for (uint32_t index = 0; index < realtime_scan_limit; index += 1) catch_damper(voices + index); if (old_contact == 0 && new_contact > 0) for (uint32_t index = 0; index < realtime_scan_limit; index += 1) if (voices[index].active && voices[index].deferred_release && !voices[index].key_down) release_voice(voices + index, DEFAULT_RELEASE_SPEED); sustain_lift = new_lift; }
...
/* Once more than four physical keys are sounding, their individual bridge-force
 * feedback is both masked and already attenuated by the dense-coupling rule.
 * Render those independent resonators voice-major so each large modal bank
 * stays in cache; ordinary polyphony retains the sample-accurate shared bus. */
static void render_dense_segment(uint32_t start, uint32_t end) {
 double cutoff_scale = active_voices > 16 ? PIANO_VERY_DENSE_MODE_CUTOFF_SCALE : PIANO_DENSE_MODE_CUTOFF_SCALE;
 for (uint32_t index = 0; index < realtime_scan_limit; index += 1) { Voice *target = voices + index; if (!target->active) continue; int strike_index = (int)target->age - target->strike_delay_samples; while (target->active_mode_count > 0 && strike_index >= 0 && (uint32_t)strike_index >= (uint32_t)(cutoff_scale * voice_mode_cutoff_at(target, target->active_mode_count - 1))) target->active_mode_count -= 1; for (uint32_t frame = start; frame < end; frame += 1) { double sample = step_voice(target, 0); if (!isfinite(sample) || !isfinite(target->bridge_output) || fabs(target->bridge_output) > 16) { memset(target, 0, sizeof(Voice)); deactivate_voice(index); break; } realtime_mix[frame] += sample; realtime_side[frame] += target->stereo_side; if (target->quiet_samples >= target->release_quiet_samples || target->inaudible_samples >= target->inaudible_window) { deactivate_voice(index); break; } } }
 sympathetic_bridge_bus = 0;
}

static void render_segment(uint32_t start, uint32_t end) { if (active_voices > DENSE_VOICE_THRESHOLD) { render_dense_segment(start, end); return; } for (uint32_t frame = start; frame < end; frame += 1) { double previous_bus = isfinite(sympathetic_bridge_bus) ? sympathetic_bridge_bus : 0, next_bus = 0; for (uint32_t index = 0; index < realtime_scan_limit; index += 1) { Voice *target = voices + index; if (!target->active) continue; double other_bridge_force = SYMPATHETIC_BRIDGE_GAIN * (previous_bus - target->bridge_output), sample = step_voice(target, other_bridge_force); if (!isfinite(sample) || !isfinite(target->bridge_output) || fabs(target->bridge_output) > 16) { memset(target, 0, sizeof(Voice)); deactivate_voice(index); continue; } realtime_mix[frame] += sample; realtime_side[frame] += target->stereo_side; next_bus += target->bridge_output; if (target->quiet_samples >= target->release_quiet_samples || target->inaudible_samples >= target->inaudible_window) deactivate_voice(index); } sympathetic_bridge_bus = isfinite(next_bus) ? next_bus : 0; } }
...
__attribute__((export_name("rt_reset"))) int reset(double sample_rate, uint32_t voice_limit) { if (!isfinite(sample_rate) || sample_rate < 32000 || sample_rate > 96000 || voice_limit < 1 || voice_limit > MAX_VOICES) return -1; memset(voices, 0, sizeof(voices)); memset(realtime_output, 0, sizeof(realtime_output)); memset(realtime_output_left, 0, sizeof(realtime_output_left)); memset(realtime_output_right, 0, sizeof(realtime_output_right)); memset(realtime_mix, 0, sizeof(realtime_mix)); memset(realtime_side, 0, sizeof(realtime_side)); realtime_rate = sample_rate; realtime_voice_limit = voice_limit; realtime_scan_limit = 0; queued_events = 0; active_voices = 0; next_serial = 0; sustain_lift = 0; una_corda_position = 0; sympathetic_bridge_bus = 0; left_microphone_state = 0; right_microphone_state = 0; left_microphone_step = 1 - exp(-TWO_PI * 12000 / sample_rate); right_microphone_step = 1 - exp(-TWO_PI * 10500 / sample_rate); voice = &offline_voice; initialize_filters(sample_rate); return 0; }
