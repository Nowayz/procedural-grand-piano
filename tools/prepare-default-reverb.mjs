...
let variant='A';
if(inputs[0]==='--preset') { inputs.shift(); variant=inputs.shift()?.toUpperCase(); }
if(!['A','B'].includes(variant)||inputs.length!==2)throw new Error('Usage: node tools/prepare-default-reverb.mjs [--preset A|B] <left-input.wav> <right-input.wav>');
const stem=`bricasti-m7-boston-hall-${variant.toLowerCase()}`;
const preset=`1 Halls ${variant==='A'?18:19} Boston Hall ${variant}`;
...
const output=new URL(`../src/impulse-responses/${stem}.wav`,import.meta.url);
await writeFile(output,wav);
const metadata={name:`Bricasti M7 Boston Hall ${variant}`,captureBy:'Samplicity',archiveVersion:'2023-10',sourcePage:'https://samplicity.com/downloads/',archiveUrl:'https://cdn.samplicity.com/downloads/Samplicity%20-%20Bricasti%20IRs%20version%202023-10.zip',archiveFiles:['L','R'].map(side=>`Samplicity - Bricasti IRs version 2023-10, left-right files, 44.1 Khz/${preset}, 44K ${side}.wav`),sourceSha256:await Promise.all(inputs.map(async file=>hash(await readFile(file)))),sha256:hash(wav),sampleRate:44100,channels:2,format:'float32',frames,durationSeconds:frames/44100,processing:'Average corresponding output channels of the left-input and right-input captures. No resampling, EQ, normalization, predelay, or tail truncation.'};
await writeFile(new URL(`../src/impulse-responses/${stem}.json`,import.meta.url),JSON.stringify(metadata,null,2)+'\n');
