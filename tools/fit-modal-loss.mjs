...
function basis(midi, velocity, partial) {
  const x = (midi - 64.5) / 43.5, y = 2 * velocity - 1, z = Math.log2(partial) / 4;
  return [
    1, x, x * x, x * x * x, y, y * y, x * y, x * x * y, x * y * y,
    z, z * z, z * z * z, x * z, x * x * z, y * z, y * y * z,
    x * y * z, x * z * z, y * z * z,
  ];
}
...
function fit(rows) {
  ...
  for (let iteration = 0; iteration < 8; iteration += 1) {
    ...
    for (let index = 0; index < size; index += 1) normal[index][index] += .05;
...
function summarize(label, rows, coefficients) {
  ...
  console.log(`${label}: ${rows.length} partial loss slopes`);
...
for (const entry of synthCache.entries) {
  const reference = referenceByFile.get(entry.file), synthesized = entry.features;
  ...
  for (let partial = 1; partial <= partialCount; partial += 1) {
    ...
    for (let frame = 2; frame < TIMES.length; frame += 1) {
      const referenceAnchor = reference.partialPowers[1]?.[partial - 1];
      const synthAnchor = synthesized.partialPowers[1]?.[partial - 1];
      const referencePower = reference.partialPowers[frame]?.[partial - 1];
      const synthPower = synthesized.partialPowers[frame]?.[partial - 1];
      if (![referenceAnchor, synthAnchor, referencePower, synthPower].every(Number.isFinite)) continue;
      const elapsed = TIMES[frame] - TIMES[1];
